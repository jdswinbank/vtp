...
\previousversion{The first version of the VOEvent Transport Protocol submitted
for IVOA approval is version 2.0. Earlier versions were described informally
in IVOA Notes, which this document supersedes.}
...
The \xmlel{iamalive} message is indicated by a role equal to
\xmlel{iamalive}. The \xmlel{<Origin~/>} element contains the
...
\xmlel{<TimeStamp~/>} element contains the date and time at which the message
...
The \xmlel{iamalive} response is an extension of the initial iamalive
message. It also has a role of \xmlel{iamalive}. It must include an
additional \xmlel{<Response~/>} element containing the IVORN of the
subscriber.  It may include a \xmlel{<Meta~/>} element with
\xmlel{<Param~/>} sub-elements which give additional information about the
subscriber or any other relevant information. \xmlel{<Param~/>} elements have
no content and must contain name and value attributes. The names and values
may be any string. The \xmlel{<TimeStamp~/>} element contains the date and
...
\xmlel{<Origin~/>} is the IVORN of the just-received VOEvent message, and an
optional \xmlel{<Result~/>} element may accompany the \xmlel{<Param~/>}
elements. \xmlel{<Result~/>} may contain any string; it is recommended that
it contain a human-readable error message if role is \xmlel{nak}. The
\xmlel{<TimeStamp~/>} element contains the date and time at which the
...
the author indicating that it has either accepted (\xmlel{ack}) or refused
(\xmlel{nak}) the VOEvent message. The connection is then closed.
...
responsibility for the message and returns an \xmlel{ack}.  Otherwise, if
validation of the signature either fails or is impossible to complete, the
message is discarded and the broker returns a \xmlel{nak}. A
\xmlel{<Result~/>} element may be included explaining that the signature
...
Transport (\S\ref{sec:transport}) message with the \xmlel{role} attribute
equal to \xmlel{authenticate} to the subscriber: see Listing
\ref{lst:authenticate} for an example. The subscriber must return a digitally
signed \xmlel{authenticate} response. The broker checks the signature as in
