...
\previousversion{This is the first public release as a Working Draft,
developed from an IVOA Note by Allan and Denny.}
...
\begin{lstlisting}[language=xml,caption=sample \texttt{iamalive} message.,
                   label=lst:iamalive]
...
\begin{lstlisting}[language=XML,caption=Sample \texttt{iamalive} response.,
...
A \texttt{nak} response should be regarded as a permanent failure state:
delivery of the VOEvent message which was met with the \texttt{nak} to the
recipient which replied with the \texttt{nak} should be aborted. If the
...
the information in the \texttt{<Meta />} element of the \texttt{nak}),
...
\begin{lstlisting}[language=XML,label=lst:ack,
                   caption=Sample VOEvent message receipt response indicating successful transmission (\texttt{ack}).]
<?xml version='1.0' encoding='UTF-8'?>
...
\begin{lstlisting}[language=XML,label=lst:nak,
                   caption=Sample VOEvent message receipt response indicating unsuccessful transmission (\texttt{nak}).]
<?xml version='1.0' encoding='UTF-8'?>
...
the author indicating that it has either accepted (\texttt{ack}) or refused
(\texttt{nak}) the VOEvent message. The connection is then closed.
...
the broker must send an \texttt{iamalive} message
(\S\ref{sec:transport:iamalive}) to the subscriber. The subscriber must reply
with an \texttt{iamalive} response (\S\ref{sec:transport:iamaliveresponse}).
If the broker does not receive \texttt{iamalive} response messages from the
...
whether the VOEvent message has been accepted (\texttt{ack}) or refused
(\texttt{nak}). The broker must not attempt to repeat delivery of the message
if a \texttt{nak} is received.

The TCP connection remains open, and the \texttt{iamalive} exchange
continues, until either the subscriber explicitly closes the connection or
stops sending \texttt{iamalive} response messages, thereby implicitly
...
Periodically, at intervals of no more than 90\,s (\S\ref{sec:maintenance}),
the subscriber should expect to receive an  \texttt{iamalive} message
(\S\ref{sec:transport:iamalive}) from the broker. The subscriber must reply
with an \texttt{iamalive} response (\S\ref{sec:transport:iamaliveresponse}).
If the subscriber does not receive \texttt{iamalive} messages from the broker
...
accepted (\texttt{ack}) or refused (\texttt{nak}) the VOEvent message.

The TCP connection remains open, and the \texttt{iamalive} exchange
continues, until either the subscriber explicitly closes the connection or
stops receiving \texttt{iamalive} messages, which indicates that the
...
opening \texttt{<} and the closing \texttt{>} of the \texttt{<VOEvent~/>}
...
responsibility for the message and returns an \texttt{ack}.  Otherwise, if
validation of the signature either fails or is impossible to complete, the
message is discarded and the broker returns a \texttt{nak}. A
\texttt{<Result~/>} element may be included explaining that the signature
...
Transport (\S\ref{sec:transport}) message with the \texttt{role} attribute
equal to \texttt{authenticate} to the subscriber: see Listing
\ref{lst:authenticate} for an example. The subscriber must return a digitally
signed \texttt{authenticate} response. The broker checks the signature as in
...
\begin{lstlisting}[language=XML,caption=Sample \texttt{authenticate} message.,
...
\begin{itemize}
    ...
    of messages which meet with a \texttt{nak} on the first attempt: VTP does
