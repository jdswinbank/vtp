...
VOEvent message was being transmistted over an author-to-broker connection,
...
The TCP conntection remains open, and the \texttt{iamalive} exchange
...
(\S\ref{sec:limit:whitelist}). If the subscriber is rejected, the connection
is terminated. Otherwise, the connection remains open, and the subcriber
...
Reconnection attempts should use a geometric back-off algorithm.
...
Note that it is now estabalished practice to distribute different descriptions
...
by the parties paritcipating in a transaction before that transaction starts.
...
time after the initial connection ahs been made, the broker may send a
...
signed \texttt{authenticate} response. The broker checks the signture as in
...
messages to be signed. If the connected is lost, forcing a reconnection, the
